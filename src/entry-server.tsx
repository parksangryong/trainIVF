...
export const getStaticRoutes = () => ["/", "/privacy", ...metroSystems.flatMap((metro) => [`/metro/${metro.id}`, ...metro.lines.map((line) => `/metro/${metro.id}/${line.id}`)])];
export const getSeo = (url: string) => {
  ...
  if (url === "/privacy") return { title: "개인정보처리방침 | 지하철 어디", description: "지하철 어디 서비스의 개인정보 및 광고 쿠키 처리 안내", canonical: "https://trainivf.com/privacy" };
